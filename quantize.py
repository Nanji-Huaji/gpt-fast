...
##### weight only bfloat16 quantized code ######


def replace_linear_weight_only_bfloat16(module):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            setattr(module, name, WeightOnlyBFloat16Linear(child.in_features, child.out_features))
        else:
            replace_linear_weight_only_bfloat16(child)


class WeightOnlyBFloat16QuantHandler:
    def __init__(self, mod):
        self.mod = mod
    ...
    def create_quantized_state_dict(self):
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                # Convert weight to bfloat16
                bfloat16_weight = mod.weight.to(torch.bfloat16)
                cur_state_dict[f"{fqn}.weight"] = bfloat16_weight

        return cur_state_dict

    def convert_for_runtime(self):
        replace_linear_weight_only_bfloat16(self.mod)
        return self.mod


class WeightOnlyBFloat16Linear(torch.nn.Module):
    __constants__ = ["in_features", "out_features"]
    in_features: int
    out_features: int
    weight: torch.Tensor
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, device=None, dtype=None) -> None:
        ...
        self.in_features = in_features
        self.out_features = out_features
        self.register_buffer("weight", torch.empty((out_features, in_features), dtype=torch.bfloat16))
...
def quantize(
    ...
    if mode == "int8":
        ...
        print("Quantizing model weights for bfloat16 weight-only symmetric per-channel quantization")
        quant_handler = WeightOnlyBFloat16QuantHandler(model)
        quantized_state_dict = quant_handler.create_quantized_state_dict()

        dir_name = checkpoint_path.parent
        base_name = checkpoint_path.name
        new_base_name = base_name.replace(".pth", f"{label}bfloat16.pth")

    else:
        raise ValueError(f"Invalid quantization mode {mode} needs to be one of [int8, int4, int4-gpptq, bfloat16]")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="int8",
        choices=["int8", "int4", "int4-gptq", "bfloat16"],
